...
def pull_from_file(filepath):
    ...
    cv2.imshow("originalimage", file)
    return file

def downsample(img, width, length):
    # nearest neighbor interpolation looks best
    output = cv2.resize(img, (length, width), fx=0.5, fy=0.5)
    cv2.imwrite("downsampled.png", output)
    return output

if __name__ == '__main__':
    img = pull_from_file("images/rainbow.jpg")
    ...
    #cv2.imwrite("converted_flower.png", new_output)
